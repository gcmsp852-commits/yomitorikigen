    ...
    while (stream.available() >= 4) {
        ...
        if (mode === ModeByte.Terminator) {
            ...
            if (stream.available() >= 20) {
                ...
                // 拡張管理部: 読取期限(bit3=0x0008), 読取者ID(bit2=0x0004), 画像データID(bit1=0x0002)
                // 順番: 読取期限32bit → 読取者ID32bit → 画像データID32bit
                var extBlockCount = 0;
                ...
                var extBitsNeeded = extBlockCount * 32 + 4; // + 終端4bit
                if (extBlockCount > 0 && stream.available() >= extBitsNeeded) {
